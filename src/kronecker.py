...
def nearest_kron_product(A, n1, m1, n2, m2):
    """
    Decomposes A=BxC where B is n1*n2, C is m1*m2
    """
    u, s, v = np.linalg.svd(rearrange(A.astype(np.complex), n1, m1, n2, m2))
    ...
    b, c = inverse_vec((max(s) ** 0.5) * u1, (n1, n2)), (max(s) ** 0.5) * inverse_vec(v1, (m1, m2))
